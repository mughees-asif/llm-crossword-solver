...
class TestCrosswordPuzzle:
    def test_init(self, puzzle):
        ...
        assert all(
            cell.value in [None, "░"] for row in current_grid.cells for cell in row
        )
    ...
    def test_undo(self, puzzle):
        ...
        clue1 = puzzle.clues[0]  # "CAT" across
        clue2 = puzzle.clues[1]  # "COW" down

        # Set first clue
        puzzle.set_clue_chars(clue1, list("CAT"))
        assert puzzle.get_current_clue_chars(clue1) == list("CAT")
        assert clue1.answered is True
        assert len(puzzle.grid_history) == 2
        assert len(puzzle.clue_history) == 1

        # Set second clue
        puzzle.set_clue_chars(clue2, list("COW"))
        assert puzzle.get_current_clue_chars(clue2) == list("COW")
        assert clue2.answered is True
        ...
        puzzle.undo()
        assert clue1.answered is True
        assert clue2.answered is False
        assert puzzle.get_current_clue_chars(clue1) == list("CAT")
        # First letter remains from clue1
        assert puzzle.get_current_clue_chars(clue2)[0] == "C"
        # Rest should be None
        assert puzzle.get_current_clue_chars(clue2)[1:] == [None, None]
        assert len(puzzle.grid_history) == 2
        assert len(puzzle.clue_history) == 1

        # Undo again
        puzzle.undo()
        assert clue1.answered is False
        assert clue2.answered is False
        assert puzzle.get_current_clue_chars(clue1) == [None, None, None]
        assert puzzle.get_current_clue_chars(clue2) == [None, None, None]
        assert len(puzzle.grid_history) == 1
        assert len(puzzle.clue_history) == 0

        # Test undoing when no moves left
    ...
    def test_reset(self, puzzle):
        ...
        # Check that characters are cleared after reset
        assert all(
            char is None for char in puzzle.get_current_clue_chars(puzzle.clues[0])
        )
        assert all(
            char is None for char in puzzle.get_current_clue_chars(puzzle.clues[1])
        )
    ...
    def test_reveal_all(self, puzzle):
        ...
        # Verify grid state
        current_grid = puzzle.current_grid
        # First cell of CAT/COW
        assert current_grid.cells[0][0].value == "C"
        # Second cell of CAT
        assert current_grid.cells[0][1].value == "A"
        # Third cell of CAT
        assert current_grid.cells[0][2].value == "T"
        # Second cell of COW
        assert current_grid.cells[1][0].value == "O"
        # Third cell of COW
        assert current_grid.cells[2][0].value == "W"
        # Second cell of TEAR
        assert current_grid.cells[1][2].value == "E"
        # Third cell of TEAR
        assert current_grid.cells[2][2].value == "A"
